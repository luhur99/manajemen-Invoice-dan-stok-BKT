...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    IF OLD.status = 'pending' THEN
      IF NEW.status NOT IN ('approved', 'rejected', 'waiting for received') THEN
        RAISE EXCEPTION 'Invalid status transition from PENDING to %.', NEW.status;
      END IF;
    ELSIF OLD.status = 'approved' THEN
      IF NEW.status NOT IN ('waiting for received', 'rejected') THEN
        RAISE EXCEPTION 'Invalid status transition from APPROVED to %.', NEW.status;
      END IF;
    ELSIF OLD.status = 'waiting for received' THEN
      IF NEW.status NOT IN ('closed', 'rejected') THEN
        RAISE EXCEPTION 'Invalid status transition from WAITING_FOR_RECEIVED to %.', NEW.status;
      END IF;
