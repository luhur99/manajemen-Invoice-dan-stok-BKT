...
DECLARE
  v_do_number TEXT;
  v_current_date TEXT := TO_CHAR(NOW(), 'YYYYMMDD');
  v_sequence_num INT;
BEGIN
  IF OLD.status = 'pending' AND NEW.status = 'approved' THEN
    -- Generate DO number
    SELECT COALESCE(MAX(SUBSTRING(do_number FROM 12)::INT), 0) + 1
    INTO v_sequence_num
    FROM public.schedules
    WHERE do_number LIKE 'DO-' || v_current_date || '-%';

    v_do_number := 'DO-' || v_current_date || '-' || LPAD(v_sequence_num::TEXT, 4, '0');
